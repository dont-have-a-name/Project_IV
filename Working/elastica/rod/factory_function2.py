...
def allocate(
    ...
    *,
    ...
    tip_radius: Optional[float] = None, #added tip_radius (optional) default set to base_radius ie uniform rest radius,
    ...
    **kwargs,
    ...
    #tip_radius = base_radius*0.1 #just tp try if it works
        for i in range(n_elements):
            radius[i] += tip_radius * (i/(n_elements-1)) + base_radius * (n_elements-i-1)/(n_elements-1)
    ...
    alpha_c = 4/3 #for circular cross sections #27.0 / 28.0
